  ...
  const get_dealer = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.error("Failed to fetch dealer data. Status:", res.status);
      }
    } catch (error) {
  ...
  return (
    <div style={{ margin: "20px" }}>
      ...
      <div style={{ marginTop: "10px" }}>
        <h1 style={{ color: "grey" }}>{dealer.full_name ? dealer.full_name : "Loading Dealer..."} {postReview}</h1>
        <h4 style={{ color: "grey" }}>
          {dealer.city ? `${dealer.city}, ${dealer.address}, Zip - ${dealer.zip}, ${dealer.state}` : "Loading Address..."}
        </h4>
      </div>
